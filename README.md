...
**FAISS**) con memoria conversacional de corto plazo. Si la pregunta no
puede responderse con la base de conocimiento disponible, el agente lo
indica explícitamente en vez de inventar una respuesta, y siempre cita el
...
- **Embeddings del RAG intercambiables, con OpenAI como valor por defecto**
  ...
  sección de instalación) — el mismo patrón intercambiable que ya usaba
  este proyecto para el LLM de chat (`LLM_PROVIDER=openai` u
  `LLM_PROVIDER=gemini`).
